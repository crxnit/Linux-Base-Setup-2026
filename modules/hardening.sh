...
configure_sysctl_hardening() {
    ...
    log_info "Applying kernel hardening parameters for architecture: $arch"
...
EOF
        ...
        log_info "ARM32 detected - skipping some advanced kernel hardening options"
    fi
...
EOF
    ...
    if sysctl -p "$sysctl_conf" >> "$LOG_FILE" 2>&1; then
        log_success "Sysctl hardening applied for $arch architecture"
    else
        log_error "Failed to apply some sysctl settings (may be architecture-specific)"
