...
_configure_ntp_service() {
    local service_name="$1"
    local package="$2"
    local conf_file="$3"
    local status_cmd="$4"

    log_info "Configuring $service_name NTP service"
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        log_info "[DRY-RUN] Would configure $service_name"
        return 0
    fi

    [[ -f "$conf_file" ]] && backup_file "$conf_file"

    # Write config (caller writes to conf_file before calling, or we use stdin)
    # Config content is written by the wrapper before this helper runs services
    ...
    eval "$status_cmd" 2>/dev/null | tee -a "$LOG_FILE" || true
...
EOF
    ...
    _configure_ntp_service "chrony" "" "$chrony_conf" "chronyc tracking"
}
...
EOF
    ...
    _configure_ntp_service "systemd-timesyncd" "" "$timesyncd_conf" "timedatectl status"
}
