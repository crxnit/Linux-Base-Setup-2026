...
set -uo pipefail
# Note: We don't use 'set -e' so the script continues on errors
# Errors are tracked and reported at the end

# Track failed components
declare -a FAILED_COMPONENTS=()

# Run a hardening step and track failures
run_step() {
    local step_name="$1"
    local step_function="$2"

    if ! "$step_function"; then
        FAILED_COMPONENTS+=("$step_name")
        log_error "$step_name failed - continuing with remaining steps"
    fi
}
...
# ============================================================================
# Root Check - Must happen before any directory/file operations
# ============================================================================
...
EOF
    ...
    # 1. System updates
    run_step "System Updates" perform_system_updates

    # 2. Configure hostname
    run_step "Hostname Configuration" configure_hostname

    # 3. Create admin user
    run_step "Admin User Creation" create_admin_user

    # 4. Setup SSH keys
    if [[ "$CREATE_ADMIN_USER" == "true" ]]; then
        run_step "SSH Key Setup" setup_ssh_keys
    fi

    # 5. Configure password policy
    run_step "Password Policy" configure_password_policy

    # 6. Configure umask
    run_step "Umask Configuration" configure_umask

    # 7. SSH hardening
    run_step "SSH Hardening" configure_ssh

    # 8. Firewall configuration
    run_step "Firewall Configuration" configure_firewall

    # 9. Kernel hardening
    run_step "Sysctl Hardening" configure_sysctl_hardening
    run_step "Protocol Hardening" disable_uncommon_protocols
    run_step "Shared Memory Security" configure_shared_memory
    run_step "Core Dump Security" configure_coredumps
    run_step "Kernel Module Security" configure_kernel_modules

    # 10. USB storage
    if [[ "$DISABLE_USB_STORAGE" == "true" ]]; then
        run_step "USB Storage Restriction" disable_usb_storage
    fi

    # 11. AppArmor
    if [[ "$CONFIGURE_APPARMOR" == "true" ]]; then
        run_step "AppArmor Configuration" configure_apparmor
    fi

    # 12. Security tools
    run_step "CrowdSec" configure_crowdsec
    run_step "Auditd" configure_auditd
    run_step "RKHunter" install_rkhunter
    run_step "Lynis" install_lynis

    # 13. Unattended upgrades
    run_step "Unattended Upgrades" configure_unattended_upgrades

    # 14. NTP and timezone
    run_step "NTP Configuration" configure_ntp

    # 15. Optional: 2FA
    if [[ "$CONFIGURE_SSH" == "true" ]]; then
        run_step "SSH Two-Factor Auth" setup_ssh_two_factor
    fi

    # 16. Rate limiting
    if [[ "$CONFIGURE_FIREWALL" == "true" ]]; then
        run_step "Rate Limiting" configure_rate_limiting
    fi
    ...
    if [[ ${#FAILED_COMPONENTS[@]} -gt 0 ]]; then
        log_section "COMPONENTS WITH ERRORS"
        log_warning "The following components encountered errors:"
        for component in "${FAILED_COMPONENTS[@]}"; do
            log_warning "  - $component"
        done
        log_warning "Review the log file for details: $LOG_FILE"
        log_warning "These components may need manual configuration."
        echo ""
    fi
